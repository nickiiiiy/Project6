$(function(){
    // нахожу табы и программирую их по клику
    $('.info-tabs-item').on('click',function(e) {
        // отменяю функции ссылок(так как табы я делал с помощью ссылок)
        e.preventDefault();

        // убираю подчеркивание           перед info-tabs-item--active не ставлю точку, так как removeClass уже указалЮ что это класс
        $('.info-tabs-item').removeClass('info-tabs-item--active');



        // Добавляю подчеркивание на активный класс
        $(this).addClass('info-tabs-item--active');

        // убираю класс актив у табов
        $('.info-tabs-content').removeClass('info-tabs-content--active');
    
        // добавляю класс актив на тот таб,на который нажал, с помощью атрибута href(так как мы передавали id на href)
        $($(this).attr('href')).addClass('info-tabs-content--active')
    });



    // подключаю слайде slick
    
    $('.info-slider, .clients-slider').slick({
        // эффект переключения
        fade: true,
         
        // кнопка назад, с помощью svg кода картинки left.svg
        ...
        // кнопка Вперед, с помощью svg кода картинки right.svg
